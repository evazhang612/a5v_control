...
def pad_sents_char(sents, char_pad_token):
    ...
    #cases: no start token
    #cases: no end token 
    #edge cases: 22 words 
    #edge case: 21 chars exact 
    # sents = [[]] #blank case 
    # sents = [[ []], [[],[]]]
    #= 21, < 21, > 21 
    ...
    for s in sents: 
        news = [] 
        missinglen = max_sen_len - len(s)
        for i in range(missinglen): 
            s.append([char_pad_token])
        for word in s: 
            missinglen = max_word_length - len(word)
            if (missinglen < 0):
                news.append(word[:max_word_length])
            elif (missinglen > 0): #longer 
                nword = []
                nword.extend(word)
                for i in range(missinglen):
                    nword += [char_pad_token]
                news.append(nword)
            else: #exactly 
                news.append(word)
