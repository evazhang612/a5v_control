...
import numpy as np#to save csv 
import pandas as pd 
...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        #save to CSV here 
        to_save = X.detach().numpy() 
        #(sentence_length, batch_size, embed_size)
        with open('save_X.csv','w') as outfile: 
            for slice_2d in to_save: 
                np.savetxt(outfile, slice_2d)

        #save the source lenghts 
        df = pd.DataFrame({'source_lengths':source_lengths})
        df.to_csv('source_lengths.csv')
